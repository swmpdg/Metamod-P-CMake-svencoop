...
void DispatchThink_Post( edict_t *UNREFERENCED( pent ) ) {
	DLL_TRACE(pfnThink, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void DispatchUse_Post( edict_t *UNREFERENCED( pentUsed ), edict_t *UNREFERENCED( pentOther ) ) {
	DLL_TRACE(pfnUse, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void DispatchTouch_Post( edict_t *UNREFERENCED( pentTouched ), edict_t *UNREFERENCED( pentOther ) ) {
	DLL_TRACE(pfnTouch, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void DispatchBlocked_Post( edict_t *UNREFERENCED( pentBlocked ), edict_t *UNREFERENCED( pentOther ) ) {
	DLL_TRACE(pfnBlocked, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void DispatchKeyValue_Post( edict_t *UNREFERENCED( pentKeyvalue ), KeyValueData *pkvd ) {
	DLL_TRACE(pfnKeyValue, P_POST, ("classname=%s keyname=%s value=%s",
			pkvd->szClassName, pkvd->szKeyName, pkvd->szValue));
	RETURN_META(MRES_IGNORED);
}
void DispatchSave_Post( edict_t *UNREFERENCED( pent ), SAVERESTOREDATA *UNREFERENCED( pSaveData ) ) {
	DLL_TRACE(pfnSave, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
int DispatchRestore_Post( edict_t *UNREFERENCED( pent ), SAVERESTOREDATA *UNREFERENCED( pSaveData ), int UNREFERENCED( globalEntity ) ) {
	DLL_TRACE(pfnRestore, P_POST, ("returning %d", META_RESULT_ORIG_RET(int)));
	// 0==Success, -1==Failure ?
	RETURN_META_VALUE(MRES_IGNORED, 0);
}
void DispatchObjectCollsionBox_Post( edict_t *UNREFERENCED( pent ) ) {
	DLL_TRACE(pfnSetAbsBox, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void SaveWriteFields_Post( SAVERESTOREDATA *UNREFERENCED( pSaveData ), const char *UNREFERENCED( pname ), 
...
void SaveReadFields_Post( SAVERESTOREDATA *UNREFERENCED( pSaveData ), const char *UNREFERENCED( pname ), 
						  void *UNREFERENCED( pBaseData ), TYPEDESCRIPTION *UNREFERENCED( pFields ), int UNREFERENCED( fieldCount ) ) {
...
void SaveGlobalState_Post( SAVERESTOREDATA *UNREFERENCED( pSaveData ) ) {
	DLL_TRACE(pfnSaveGlobalState, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void RestoreGlobalState_Post( SAVERESTOREDATA *UNREFERENCED( pSaveData ) ) {
...
BOOL ClientConnect_Post( edict_t *UNREFERENCED( pEntity ), const char *UNREFERENCED( pszName ), const char *UNREFERENCED( pszAddress ), char szRejectReason[ 128 ]  ) {
...
void ClientDisconnect_Post( edict_t *UNREFERENCED( pEntity ) ) {
	DLL_TRACE(pfnClientDisconnect, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void ClientKill_Post( edict_t *UNREFERENCED( pEntity ) ) {
	DLL_TRACE(pfnClientKill, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void ClientPutInServer_Post( edict_t *UNREFERENCED( pEntity ) ) {
	DLL_TRACE(pfnClientPutInServer, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void ClientCommand_Post( edict_t *UNREFERENCED( pEntity ) ) {
	DLL_TRACE(pfnClientCommand, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void ClientUserInfoChanged_Post( edict_t *UNREFERENCED( pEntity ), char *UNREFERENCED( infobuffer ) ) {
	DLL_TRACE(pfnClientUserInfoChanged, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void ServerActivate_Post( edict_t *UNREFERENCED( pEdictList ), int UNREFERENCED( edictCount ), int UNREFERENCED( clientMax ) ) {
...
void PlayerPreThink_Post( edict_t *UNREFERENCED( pEntity ) ) {
	DLL_TRACE(pfnPlayerPreThink, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void PlayerPostThink_Post( edict_t *UNREFERENCED( pEntity ) ) {
...
void PlayerCustomization_Post( edict_t *UNREFERENCED( pEntity ), customization_t *UNREFERENCED( pCust ) ) {
	DLL_TRACE(pfnPlayerCustomization, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void SpectatorConnect_Post( edict_t *UNREFERENCED( pEntity ) ) {
	DLL_TRACE(pfnSpectatorConnect, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void SpectatorDisconnect_Post( edict_t *UNREFERENCED( pEntity ) ) {
	DLL_TRACE(pfnSpectatorDisconnect, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void SpectatorThink_Post( edict_t *UNREFERENCED( pEntity ) ) {
	DLL_TRACE(pfnSpectatorThink, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void Sys_Error_Post( const char *UNREFERENCED( error_string ) ) {
	DLL_TRACE(pfnSys_Error, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}

// from SDK pm_shared/pm_shared.c:
void PM_Move_Post( struct playermove_s *UNREFERENCED( ppmove ), int UNREFERENCED( server ) ) {
	DLL_TRACE(pfnPM_Move, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void PM_Init_Post( struct playermove_s *UNREFERENCED( ppmove ) ) {
	DLL_TRACE(pfnPM_Init, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
char PM_FindTextureType_Post( char *UNREFERENCED( name ) ) {
	DLL_TRACE(pfnPM_FindTextureType, P_POST, (""));
	RETURN_META_VALUE(MRES_IGNORED, '\0');
}

// from SDK dlls/client.cpp:
void SetupVisibility_Post( edict_t *UNREFERENCED( pViewEntity ), edict_t *UNREFERENCED( pClient ), 
						   unsigned char **UNREFERENCED( pvs ), unsigned char **UNREFERENCED( pas ) ) {
	DLL_TRACE(pfnSetupVisibility, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void UpdateClientData_Post ( const struct edict_s *UNREFERENCED( ent ), 
							 int UNREFERENCED( sendweapons ), struct clientdata_s *UNREFERENCED( cd ) ) {
	DLL_TRACE(pfnUpdateClientData, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
int AddToFullPack_Post( struct entity_state_s *UNREFERENCED( state ), int UNREFERENCED( e ), edict_t *UNREFERENCED( ent ), 
...
void CreateBaseline_Post( int UNREFERENCED( player ), int UNREFERENCED( eindex ), struct entity_state_s *UNREFERENCED( baseline ), 
						  struct edict_s *UNREFERENCED( entity ), int UNREFERENCED( playermodelindex ), 
						  vec3_t UNREFERENCED( player_mins ), vec3_t UNREFERENCED( player_maxs ) ) {
...
int GetWeaponData_Post( struct edict_s *UNREFERENCED( player ), struct weapon_data_s *UNREFERENCED( info ) ) {
	DLL_TRACE(pfnGetWeaponData, P_POST, ("returning %d", META_RESULT_ORIG_RET(int)));
	RETURN_META_VALUE(MRES_IGNORED, 1);
}
void CmdStart_Post( const edict_t *UNREFERENCED( player ), const struct usercmd_s *UNREFERENCED( cmd ), unsigned int UNREFERENCED( random_seed ) ) {
	DLL_TRACE(pfnCmdStart, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
void CmdEnd_Post ( const edict_t *UNREFERENCED( player ) ) {
	DLL_TRACE(pfnCmdEnd, P_POST, (""));
	RETURN_META(MRES_IGNORED);
}
int ConnectionlessPacket_Post( const struct netadr_s *UNREFERENCED( net_from ), const char *UNREFERENCED( args ), 
...
int GetHullBounds_Post( int UNREFERENCED( hullnumber ), float *UNREFERENCED( mins ), float *UNREFERENCED( maxs ) ) {
...
int InconsistentFile_Post( const edict_t *UNREFERENCED( player ), const char *UNREFERENCED( filename ), char *disconnect_message ) {
...
int ShouldCollide_Post(edict_t *UNREFERENCED( pentTouched ), edict_t *UNREFERENCED( pentOther )) {
	NEWDLL_TRACE(pfnShouldCollide, P_POST, ("returning %d", META_RESULT_ORIG_RET(int)));
	RETURN_META_VALUE(MRES_IGNORED, 1);
}

// Added 2005-08-11 (no SDK update)
void CvarValue_Post(const edict_t *UNREFERENCED( pEdict ), const char *UNREFERENCED( szValue ))
{
